...
Choreographs the player-initiated d20 roll as a thrown die — a scripted parabolic
arc with in-flight tumble, decaying bounces, continuous deceleration, and camera
framing that keeps the die in view and the resting face legible. The animation is
presentation only: it always settles on the authoritative server-rolled face,
plays regardless of reduced-motion preferences, and degrades gracefully when 3D
