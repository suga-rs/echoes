...
The header dropdown, opened from the logged-in player's username/avatar, that
consolidates per-user actions (go to profile, open settings, log out) while
keeping settings reachable for logged-out players via a standalone control.
