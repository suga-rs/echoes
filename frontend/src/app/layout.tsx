...
const PREFERENCIAS_PRE_PAINT = `(function(){try{var raw=localStorage.getItem('aventuras-settings');if(!raw)return;var s=(JSON.parse(raw)||{}).state||{};var fonts={serif:'"Crimson Text", Georgia, serif',sans:'Inter, system-ui, sans-serif',dyslexic:'"Atkinson Hyperlegible", "Comic Sans MS", sans-serif'};var sizes={sm:'0.95rem',md:'1.05rem',lg:'1.2rem'};var r=document.documentElement;if(fonts[s.narrativaFont])r.style.setProperty('--font-narrativa',fonts[s.narrativaFont]);if(sizes[s.narrativaSize])r.style.setProperty('--narrativa-size',sizes[s.narrativaSize]);}catch(e){}})();`;
