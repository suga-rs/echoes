...
| ------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 3.4.0   | 2026-06-24 | **Consistencia visual de NPCs (anclaje por texto).** Los NPCs/enemigos cambiaban de aspecto entre imágenes porque no tenían ancla: solo el protagonista llevaba descripción visual al prompt de imagen, y la apariencia de los NPCs entraba ad-hoc en `descripcion_escena_en` cada turno. Ahora `npc_encontrado` gana `descripcion_visual_en` (aspecto canónico en inglés, capturado UNA vez al introducir el NPC y persistido en el modelo `NPC`), y `generar_imagen` gana `npcs_en_escena` (nombres de NPCs conocidos presentes en la escena). El servicio resuelve esos nombres contra `world_state.npcs` (match case/whitespace-insensitive, ignorando los no resueltos o sin descripción) y `build_image_prompt` inyecta hasta `MAX_NPCS_ANCLADOS` (=2) descripciones canónicas como `Also present: …` entre el protagonista y la escena. Sin generaciones de imagen extra ni cambio de mecanismo (sigue siendo un solo `images.edit`): el delta es texto en el prompt. Retrocompatible: NPCs/partidas previos sin el campo se tratan como "sin ancla". |
| 3.3.0   | 2026-06-24 | **Intención vs. resolución del jugador.** El narrador obedecía acciones que afirmaban su propio desenlace ("…y lo mato"): ponía `requiere_tirada` en `null` y narraba el resultado dictado por el jugador, salteando el d20. Se agrega una regla dura a `SYSTEM_PROMPT_TURNO` (INTENCIÓN vs. RESOLUCIÓN): el texto del jugador declara lo que INTENTA, nunca el resultado; si afirma una resolución, el narrador la ignora y resuelve solo el intento. Un intento incierto SIEMPRE requiere tirada sin importar la redacción; un intento imposible se narra como intento fallido sin tirada. La Regla #6 se acota para excluir la resolución, y `build_turno_user_prompt` re-enmarca el header como "INTENTO DEL JUGADOR (su intención, NO el desenlace)". Solo prompt: sin cambios de schema, código ni datos. Aplica solo a la acción de turno (creación y opciones generadas fuera de alcance). |
| 3.2.0   | 2026-06-24 | **Puntos de vida + condiciones (`consecuencia`).** El fracaso deja de ser solo narrativo: el turno gana un campo `consecuencia` obligatorio y nullable, sibling de `requiere_tirada` (independiente de él, así una trampa/veneno daña sin tirada previa). El narrador declara una BANDA de severidad de `dano` (rasguno/leve/grave/severo/mortal) —nunca PV crudos—, puede aplicar/quitar una condición (`envenenado`/`sangrando`/`aturdido`/`exhausto` con efecto `desventaja` o `dano_por_turno` y una duración), declarar `descanso`, o `curar_pocion` (consumida del inventario). El sistema es dueño del número: mapea la banda a una fracción de `pv_max`, y `pv_max` se deriva de la constitución en la creación. Una condición con `desventaja` hace que la resolución del d20 tire 2d20 y se quede con el peor. Al llegar a 0 PV la partida termina en `fracaso`. Retrocompatible: partidas previas deserializan a tope de vida (constitución 10) y sin condiciones. |
| 3.1.0   | 2026-06-23 | **`requiere_tirada` pasa a requerido + nullable.** El modelo casi nunca declaraba tiradas porque el campo era opcional (camino de menor resistencia: omitirlo y narrar el desenlace directo). Ahora es obligatorio en el schema del turno (valor `null` u objeto), forzando una decisión consciente por turno. Se refuerza el system prompt: ante una acción que *podría fallar* el default es declarar tirada, con ejemplos concretos y la directiva de no narrar "lográs/fallás" sin tirar primero. |
| 3.0.0   | 2026-06-23 | **Tiradas de dado d20 + ficha de seis atributos.** El narrador deja de ser autor único del desenlace: cuando la acción es incierta declara una tirada (`requiere_tirada` = {habilidad ∈ las seis, banda ∈ trivial/facil/media/dificil/heroica}) en lugar de narrar el resultado. El sistema tira un d20 real server-side, suma el modificador de la habilidad y compara contra el DC fijo de la banda (5/10/15/20/25), clasificando en exito_critico/exito/fracaso/fracaso_critico (nat-20 y nat-1 son críticos siempre). El turno con tirada es de DOS llamadas: fase 1 declara, el sistema tira, y `SYSTEM_PROMPT_RESOLUCION` narra el desenlace honrando el tier. La creación agrega los seis atributos (3-18) sesgados a la descripción. Acciones triviales/imposibles siguen en una sola llamada. |
| 2.2.0   | 2026-06-19 | **Idioma por campo, bidireccional.** Los campos cortos tipo etiqueta (`objetivo`, `inventario`, `opciones`) salían a veces en inglés porque el idioma se afirmaba de forma global y prosa-céntrica, sin fijarse por campo, y los campos `_en` vecinos cebaban inglés. Se agrega a ambos system prompts una regla dura bidireccional: TODO campo de texto va en español rioplatense SALVO los que terminan en `_en`, que van en inglés. Se refuerza con `description` en el JSON schema de las properties que fugaban (`objetivo`, `inventario_inicial`, `agregar_inventario`/`quitar_inventario`, `opciones`). Acompaña un script one-off de remediación para partidas previas con el `objetivo`/inventario en inglés. |
| 2.1.0   | 2026-06-16 | **Semillas creativas en creación.** Para romper la repetición entre partidas (mismos nombres tipo "Elias", mismas premisas y tono), se muestrea por juego una `SemillaCreativa` (nombre/premisa/tono/apertura) de pools curados por género y se inyecta en el prompt de creación como *inspiración, no guion*. El jugador puede dar premisa y tono opcionales: si los da se honran fielmente; si no, los cubre la seed. El nombre de la seed es solo respaldo (el nombre del jugador en su descripción manda). Sin cambios en el prompt de turno. |
